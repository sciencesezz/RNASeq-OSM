######################################Deseq2 mlcm test############################
getwd()
setwd("/mnt/data/home/sarahsczelecki/mlcm/output-files")
library(tidyverse)
library(ggplot2)
library(tidyr)
...
## load count file
count_file <- read.csv("/mnt/data/home/sarahsczelecki/mlcm/mlcm_total_counts.csv", sep=',', header = TRUE, row.names = "Geneid")
metadata <- read.csv("/mnt/data/home/sarahsczelecki/mlcm/metadata.csv", sep=',', header = TRUE)
...
write.csv(count_file_cpm, "mlcm_logcpmc.csv", row.names = TRUE)
...
order_group <- c("Control", "Sxcd", "Adenoma")
...
# Contrasts
contrasts_list <- list(
 Sxcd_vs_Control = c("group", "Sxcd", "Control"),
 Adeno_vs_Control = c("group", "Adenoma", "Control"))
...
write.csv(count_file_cpm_anno, "mlcm_logcpmc_anno.csv", row.names = TRUE)
